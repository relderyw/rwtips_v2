...
export const BankrollManager: React.FC<BankrollManagerProps> = ({ userEmail }) => {
  ...
  // Specific for Multipla selections
  const [tempSelectionOdd, setTempSelectionOdd] = useState('');
  ...
  const addSelection = () => {
    if (!formData.liga || !formData.jogador1 || !formData.mercado || !tempSelectionOdd) {
      alert("Preencha os dados da seleção (Liga, Jogador 1, Mercado, Odd)");
      return;
    }
    const newSelection: BetSelection = {
      ...
      odds: parseFloat(tempSelectionOdd.replace(',', '.'))
    };
  ...
  const handleBetSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.type === 'simples') {
      ...
      if (formData.selections.length < 2) {
        alert("Adicione pelo menos 2 seleções para uma aposta múltipla. Para aposta simples, mude o tipo.");
        return;
      }
      if (!formData.stake || !formData.odds) {
        alert("Preencha o valor da aposta e a odd total.");
        return;
      }
    }

    try {
      const stakeString = formData.stake.toString().replace(',', '.');
      const oddsString = formData.odds.toString().replace(',', '.');

      const stake = parseFloat(stakeString);
      const odds = parseFloat(oddsString);

      if (isNaN(stake) || isNaN(odds)) {
        alert("Valor da aposta ou Odd inválida. Use apenas números e ponto/vírgula.");
        return;
      }

      const betData: any = {
        type: formData.type || 'simples',
        stake,
        odds,
                ...
                <div key={bet.id} className="group relative flex flex-col md:flex-row items-center gap-4 bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 p-4 rounded-2xl transition-all">
                  ...
                  <div className="flex-1 w-full min-w-0 grid grid-cols-2 md:grid-cols-5 gap-4 items-center">
                    ...
                    <div className="md:col-span-2">
                      {bet.type === 'multipla' && bet.selections && bet.selections.length > 0 ? (
                        <div className="space-y-1.5 mt-1">
                          <p className="text-[10px] font-black text-white/40 uppercase mb-1">{bet.selections.length} Seleções</p>
                          {bet.selections.map((sel, idx) => (
                            <div key={idx} className="bg-white/5 p-1.5 rounded-lg border border-white/5 flex flex-col gap-0.5">
                              <div className="flex justify-between items-center">
                                <span className="text-[8px] font-bold text-white/60">{sel.liga}</span>
                                <span className="text-[8px] font-bold text-emerald-500">{sel.mercado}</span>
                              </div>
                              <div className="text-[9px] font-medium text-white/80">{sel.jogador1} <span className="text-rose-500">vs</span> {sel.jogador2}</div>
                              <div className="text-[8px] font-bold text-indigo-400 text-right">@{sel.odds}</div>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center gap-2 text-[10px] font-black uppercase text-white/40 mb-1">
                            ...
                            {bet.jogador2 && <><span className="text-rose-500">vs</span><span>{bet.jogador2}</span></>}
                          </div>
      ...
      {activeTab === 'new-bet' && (
        <div className="max-w-2xl mx-auto bg-[#0a0a0c] border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleBetSubmit} className="space-y-6">
            ...
            {/* TYPE SELECTOR - ONLY IF NOT EDITING (OR IF WE SUPPORT EDITING TYPE) */}
            <div className="flex bg-black/40 p-1 rounded-xl border border-white/10 w-fit mx-auto mb-2">
              <button
                type="button"
                onClick={() => setFormData({ ...formData, type: 'simples' })}
                className={`px-6 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${formData.type === 'simples' ? 'bg-emerald-500 text-black shadow-lg' : 'text-white/40 hover:text-white'}`}
              >
                Simples
              </button>
              <button
                type="button"
                onClick={() => setFormData({ ...formData, type: 'multipla' })}
                className={`px-6 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${formData.type === 'multipla' ? 'bg-indigo-500 text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
              >
                Múltipla
              </button>
            </div>

            {/* SELECTIONS LIST (MULTIPLA) */}
            {formData.type === 'multipla' && formData.selections.length > 0 && (
              <div className="bg-white/[0.02] rounded-2xl border border-white/5 p-4 space-y-3">
                <h4 className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-2">Seleções ({formData.selections.length})</h4>
                  ...
                  <div key={idx} className="flex items-center justify-between bg-black/40 p-3 rounded-xl border border-white/5">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[9px] font-black bg-white/10 px-1.5 py-0.5 rounded text-white/60 uppercase">{sel.liga}</span>
                        <span className="text-[9px] font-bold text-emerald-500 uppercase">{sel.mercado}</span>
                      </div>
                      <div className="text-xs font-bold text-white">{sel.jogador1} <span className="text-rose-500">vs</span> {sel.jogador2}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-black text-indigo-400">@{sel.odds}</span>
                      <button type="button" onClick={() => removeSelection(idx)} className="p-2 hover:bg-rose-500/20 text-white/20 hover:text-rose-500 rounded-lg transition-colors">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="bg-white/[0.01] rounded-2xl border border-white/5 p-4 md:p-6 space-y-6 relative">
              ...
              {/* SELECTION ODD (If Multipla) or STAKE/ODD (If Simples) */}
              {formData.type === 'simples' ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
                  <div className="space-y-2">
                    <label className="text-[9px] font-black text-white/30 uppercase tracking-widest ml-3">Odd da Seleção</label>
                    <input
                      type="number"
                      step="0.01"
                      value={tempSelectionOdd}
                      onChange={(e) => setTempSelectionOdd(e.target.value)}
                      className="w-full bg-black border border-white/10 rounded-2xl px-5 py-4 text-xl font-black text-indigo-400 outline-none focus:border-emerald-500 transition-colors"
                      placeholder="1.90"
                    />
                  </div>
                  <button
                    ...
                    className="w-full bg-white/10 hover:bg-emerald-500 hover:text-black text-emerald-500 font-bold uppercase tracking-widest py-4 rounded-xl transition-all flex items-center justify-center gap-2"
                  >
            ...
            {/* FINAL BET INFO (MULTIPLA) */}
            {formData.type === 'multipla' && (
              <div className="bg-indigo-500/5 rounded-2xl border border-indigo-500/20 p-6 grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-white/30 uppercase tracking-widest ml-3">Stake Total (R$)</label>
                  <input
                    type="number"
                    value={formData.stake}
                    onChange={(e) => setFormData({ ...formData, stake: e.target.value })}
                ...
                <div className="space-y-2">
                  <label className="text-[9px] font-black text-white/30 uppercase tracking-widest ml-3">Odd Final</label>
                  <input
                    type="number"
                    step="0.01"
                    value={formData.odds}
              ...
              <div className="grid grid-cols-3 gap-2">
                {['aguardando', 'green', 'red', 'meio-green', 'meio-red', 'reembolso'].map((status) => (
                  <button
                    ...
                    className={`px-2 py-3 rounded-xl text-[9px] font-black uppercase tracking-wider border transition-all ${formData.resultado === status
