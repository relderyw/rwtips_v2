...
export const LiveMatchCard: React.FC<LiveMatchCardProps> = ({ match, potential, historicalGames, onDetailClick }) => {
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        // ATENÇÃO: Overflow visible para que o pop-up das bolinhas flutue livremente
        overflow: 'visible' 
      }}
    >
      {isSignaled && (
        <div className="absolute top-3.5 right-3.5 z-20 flex flex-col items-end gap-1.5">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg shadow-lg backdrop-blur-md border" 
               style={{ backgroundColor: `${theme.color}20`, borderColor: `${theme.color}40` }}>
             <i className={`fa-solid ${theme.icon} text-[10px] animate-pulse`} style={{ color: theme.color }}></i>
             <span className="text-[10px] font-black uppercase tracking-widest leading-none" style={{ color: theme.color }}>
                {theme.label}
             </span>
          </div>
        </div>
