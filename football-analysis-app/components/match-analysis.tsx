...
export function MatchAnalysis({ matchId, homeTeamId, awayTeamId, tournamentId }: MatchAnalysisProps) {
    ...
    const fetchTournaments = async () => {
      try {
        const [homeResponse, awayResponse] = await Promise.all([
          fetch(`/api/team-tournaments?teamId=${homeTeamId}`),
          fetch(`/api/team-tournaments?teamId=${awayTeamId}`),
        ])
