...
export const analyzeMatchPotential = (p1Name: string, p2Name: string, gamesData: any): string => {
  let games: HistoryMatch[] = [];
  ...
  // 1. HT PRO SNIPER
  if (p1.over05HT === 100 && p1.avgGoalsHT >= 1.5 &&
      p2.over05HT === 100 && p2.avgGoalsHT >= 1.5) {
      return 'ht_pro';
  }
  ...
  if (avgOver25 >= 97 && avgOver35 >= 75 && avgBtts >= 88 && avgGoalsFT >= 4.0) {
      return 'ft_pro';
  }

  // 3. BTTS HT PRO
  if (p1.avgGoalsHT >= 1.5 && p2.avgGoalsHT >= 1.5 && p1.bttsHT > 80 && p2.bttsHT > 80) {
      return 'btts_pro_ht';
  }

  // 4. DOMINANTE (Casa ou Fora)
  if (p1.winRate >= 60 && p1.avgGoalsFT >= 2.0 && p2.winRate <= 40 && p2.avgGoalsFT <= 1.5) {
      return 'casa_pro';
  }
  if (p2.winRate >= 60 && p2.avgGoalsFT >= 2.0 && p1.winRate <= 40 && p1.avgGoalsFT <= 1.5) {
      return 'fora_pro';
  }

  // 5. ENGINE PRO (Casa ou Fora)
  if (p1.avgGoalsHT >= 2.0 && p1.avgGoalsFT >= 2.5 && p2.avgGoalsHT <= 1.0 && p2.avgGoalsFT <= 1.8) {
      return 'casa_engine_pro';
  }
  if (p2.avgGoalsHT >= 2.0 && p2.avgGoalsFT >= 2.5 && p1.avgGoalsHT <= 1.0 && p1.avgGoalsFT <= 1.8) {
      return 'fora_engine_pro';
  }

  // 6. ELITE CLASH
  if (p1.winRate >= 50 && p2.winRate >= 50) return 'top_clash';
