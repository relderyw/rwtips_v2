...
const STRATEGY_DESCRIPTIONS: Record<string, string> = {
  ...
  btts_pro_ft: "BTTS FT PRO: Ambos jogadores com média >= 2.0 gols e alta taxa de 'Ambas Marcam' no FT.",
...
const App: React.FC = () => {
  ...
  const [showThermometers, setShowThermometers] = useState(true);
  ...
  useEffect(() => {
    if (isLoggedIn) {
      // Se estiver no módulo FIFA, inicia sync
      if (selectedModule === 'fifa') {
        initData();
        ...
        // Se estiver em outro módulo (ou nenhum), para loading (para não travar)
        // e opcionalmente limpa dados se necessário
        setIsLoadingData(false);
      }
    }
  ...
  return (
    <div className={`min-h-screen bg-[#030303] text-white selection:bg-emerald-500 selection:text-black`}>
      ...
      <div className={`${(!isLoggedIn || (isLoggedIn && !selectedModule) || isAdminView) ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'} transition-all duration-700`}>
        <div className="fixed top-6 right-6 z-[6000] flex flex-col gap-4 pointer-events-none">
          {notifications.map(n => (
            // Só mostra notificação se estiver no FIFA
            selectedModule === 'fifa' && <GoalToast key={n.id} notification={n} onClose={removeNotification} />
          ))}
        </div>
                ...
                <>
                  {activeMainTab === 'radar' ? (
                    <>
                      ...
                      {/* --- LEAGUE THERMOMETERS DASHBOARD --- */}
                      <div className="space-y-4 mb-4 mt-8 pt-8 border-t border-white/[0.04]">
                        <div 
                          className="flex items-center justify-between cursor-pointer bg-white/[0.02] border border-white/5 rounded-2xl p-4 hover:bg-white/[0.04] transition-colors" 
                          onClick={() => setShowThermometers(!showThermometers)}
                        >
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-orange-500/10 text-orange-500 flex items-center justify-center border border-orange-500/20">
